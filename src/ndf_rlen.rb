...
for type in types
	...
	vdecl = ''; vref = uref
    when :SI2
	ctype, neg, suf = 'N_SI2', '負値または', 'i2'
	negchk = 'udata[iu] < 0 || '
	vtype, uref = 'int', 'udata[iu]'
	vdecl = ''; vref = uref
    when :SI4
	ctype, neg, suf = 'N_SI4', '負値または', 'i4'
	negchk = 'udata[iu] < 0 || '
	vtype, uref = 'int', 'udata[iu]'
	vdecl = ''; vref = uref
    when :R4
	ctype, neg, suf = 'float', '負値または', 'r4'
	negchk = 'udata[iu] < 0.0 || '
	vtype, uref = 'long', 'ROUND(udata[iu])'
	vdecl = "int v;\n\tv = ROUND(udata[iu]);"; vref = 'v'
    when :R8
	ctype, neg, suf = 'double', '負値または', 'r8'
	negchk = 'udata[iu] < 0.0 || '
	vtype, uref = 'long', 'ROUND(udata[iu])'
	vdecl = "int v;\n\tv = ROUND(udata[iu]);"; vref = 'v'
    end
    ...
    print <<EOF
/** @brief #{ctype} 配列を連長圧縮する
 * @retval 非負 圧縮結果のバイト数
 * @retval -1 圧縮結果は cbuf_nbytes バイトを超える
 * @retval -2 入力データに#{neg}大きすぎる値がある
 */
	N_SI4
nus_compress_rlen_#{suf}(const #{ctype} *udata, /**< 入力データ */
...
/** @brief 連長圧縮データから #{ctype} 配列に展開する
 * @retval 非負 展開後の要素数
 * @retval -4 データが長過ぎて出力先に格納できない
 * @retval -6 圧縮データが壊れている (最初のバイト値がデータ最大値を超える)
 */
	N_SI4
nus_decompress_rlen_#{suf}(const N_UI1 *compressed, /**< 圧縮データ */
		N_UI4 maxv, /**< データの最大値 */
