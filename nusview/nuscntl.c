...
static char rcsid[] = "$Id: nuscntl.c,v 1.7 2007-03-17 12:13:09 suuchi04 Exp $";

#define EMAP 0
#define DMAP 1

struct nuscntl_opts{
    int (*operator)(struct nus_dims_t *dp, struct nuscntl_opts *op);
    int put_title;
    int is_radar;
    int edflag;
};

int nusdims_cntl_rd(struct nus_dims_t *dp, struct nuscntl_opts *op);
int nusdims_cntl_edmap(struct nus_dims_t *dp, struct nuscntl_opts *op);



	void
opt_clear(struct nuscntl_opts *op)
{
    op->operator =nusdims_cntl_rd; 
    op->put_title = 0;
}

	void
option(struct nuscntl_opts *op, const char *str)
{
	if (str == NULL)
		return;
	if (*str == '-')
	...
	case 't':
            ...
            break;
	case 'm':
		op->operator = nusdims_cntl_edmap;
	...
	int
nusdims_cntl_rd(struct nus_dims_t *dp, struct nuscntl_opts *op)
{
    int	r, n = 2;
    int     array_num;
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_MEMBER_NUM)] %d, %d\n", r, array_num);
        return r;
    }
    if((member_list = (char*)malloc(4 * member_num)) == NULL){
        eprintf(HTTP_ERR, "nusdas_inq_cntl2: malloc error \n");
        return -10;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_MEMBER_LIST)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_VALIDTIME_NUM)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_VALIDTIME_LIST)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_PLANE_NUM)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_PLANE_LIST)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_ELEMENT_NUM)] %d, %d\n", r, array_num);
        return r;
    }
    ...
    r = nusdas_inq_cntl2(dp->type.type1, dp->type.type2, dp->type.type3,
                         ...
                         (N_SI4*)&array_num);
    if (r != array_num) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_inq_cntl2(N_ELEMENT_LIST)] %d, %d\n", r, array_num);
        return r;
    }

    r = nusdas_grid2(dp->type.type1, dp->type.type2, dp->type.type3,
		    &dp->base, dp->member, &dp->valid, &dp->valid2,
		     proj, (N_SI4*)gridsize, ginfo, mean, N_IO_GET);
    if (r != 0) {
        eprintf(HTTP_ERR, "nusdas error [nusdas_grid] %d\n", r);
        return r;
    }
    ...
    distance[0] = ginfo[4];
    distance[1] = ginfo[5];
    stand_latlon[0] = ginfo[6];
    stand_latlon[1] = ginfo[7];
    stand_latlon[2] = ginfo[8];
    stand_latlon[3] = ginfo[9];
    spare_latlon[0] = ginfo[10];
    spare_latlon[1] = ginfo[11];
    spare_latlon[2] = ginfo[12];
    spare_latlon[3] = ginfo[13];
...
/*-------------------------------------------------------------------------*/
	int
nusdims_cntl_edmap(struct nus_dims_t *dp, struct nuscntl_opts *op)
{
    int	    r;
    int     array_num;
    int     indx_size;
    ...
    if (r != array_num) {
		...
		"[nusdas_inq_cntl2(N_INDX_SIZE)] %d, %d\n", r, array_num);
        return r;
    }
    if (op->edflag == EMAP){
	    query = N_ELEMENT_MAP;
    } else if (op->edflag == DMAP){
	    query = N_DATA_MAP;
    }
